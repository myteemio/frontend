...
export default function Test() {
  ...
  function createEvent() {
    alert(`Create event! with the followng activites: ${addedActivities.map((v, i) => v.activity.title).join(',')}`);
  }
      ...
      <div className={styles.container}>
        ...
        >
              ...
              {adjustedActivities.map((item, i) => (
                <ActivityDropped
                  ...
                  style={{
                    ...
                    border: item.isIncorrect ? '2px solid red' : 'none',
                  }}
          ...
          <div className={styles.rightside}>
            <p className={styles.rightsidetitle}>AKTIVITETER</p>
            ...
            <div className={styles.rightsideStart} onClick={() => createEvent()}>
              OPRET EVENT
            </div>
          </div>
          <DragOverlay
  ...
  function applyDragToList(item: HTMLElement) {
    item.classList.add('draggedToList');
    item.classList.remove('draggedToTable');

    const img = item.querySelector('[class*="activityImage"]') as HTMLElement;
    if (img) img.style.display = 'block';
  }

  function applyDragToTable(item: HTMLElement) {
    item.classList.add('draggedToTable');
    item.classList.remove('draggedToList');
    ...
    if (over && over.id) {
      ...
      if (item && overElement) {
        if (over.id === 'activitytimeslotsdroppable') {
          ...
          // Add the styling of when dragged into the area
          // Update the component here..
          applyDragToTable(item);

          return;
        }

        if (over.id === 'activitydroppable') {
          applyDragToList(item);
          return;
        }
      }
    }
...
function ActivityBeingDragged({
  ...
  timetable: { hours: number; startTime: number };
}) {
  const { setNodeRef, listeners, attributes } = useDraggable({
    id: id,
    data: {
  ...
  if (details.DraggedFrom === 'activitydroppable') {
    ...
    var timeslot = convertPositionAndHeightToTimeslots(
      details.height,
      details.top,
      timetable.hours,
      timetable.startTime
    ...
    return (
      ...
      >
        <div className={styles.activityinner}>
          <p className={classNames(styles.activitytitle, styles.activitytitlesmall)}>{details.activity.title}</p>
        </div>
      </div>
    );
  }
...
function ActivityDropped({
  ...
  const internalstyle: any = {
    background: isPlaceholder ? 'lightgray' : 'none',
    boxSizing: isPlaceholder ? 'border-box' : '',
    ...style,
  };
  ...
  return (
    <div style={internalstyle}>
      ...
      >
        ...
        >
          <p style={{ margin: 0, padding: 0, visibility: isPlaceholder ? 'hidden' : 'visible', color: 'black' }}>
            {details.activity.title} - {timeslot.start} til {timeslot.end} ({duration} timer)
          </p>
        </div>
      </div>
